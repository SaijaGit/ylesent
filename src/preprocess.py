...
def run(): 
  ...
  data = pd.read_csv('./data/raw/fi.csv')
  #print(data.head())
  #print(data.info())
  ...
  data['text'] = data['text'].apply(lambda x: re.sub(r'[^a-zA-ZäöåÄÖÅ\s]', '', x)) # remove from special characters, except letters and spaces
  ...
  data[['text_for_model', 'label']].to_csv('./data/preprocessed/training_preprocessed.csv', index=False)
